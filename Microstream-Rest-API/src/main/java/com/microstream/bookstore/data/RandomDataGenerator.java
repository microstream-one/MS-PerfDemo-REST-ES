...
public interface RandomDataGenerator
{
	...
	public static class Default implements RandomDataGenerator
	{
		...
		public DataMetrics generate()
		{
			...
			final List<CountryData> countries = locales.parallelStream().map(this::createCountry).collect(toList());
		...
		private List<Customer> createCustomers(
			...
			final City city)
		{
			return this.randomRange(this.dataAmount.maxCustomersPerCity()).mapToObj(
				i -> Customer.New(
					this.customerId.incrementAndGet(),
					countryData.faker.name().fullName(),
					this.createAddress(city, countryData.faker))).collect(toList());
		}
			...
			final List<CountryData> countries)
		{
			...
			countries.parallelStream().forEach(country ->
			{
				...
				final List<Book> books = IntStream.range(0, this.dataAmount.maxBooksPerCountry()).mapToObj(
					i -> country.faker.book().title()).map(
						title -> this.createBook(country, genres, publishers, authors, language, title)).collect(
							toList());
		...
		{
			return this.randomRange(this.dataAmount.maxGenres()).mapToObj(
				i -> this.faker.book().genre()).distinct().map(Genre::New).collect(toList());
		}
		
		private List<Publisher> createPublishers(
			final CountryData countryData)
		{
			return this.randomRange(this.dataAmount.maxPublishersPerCountry()).mapToObj(
				i -> countryData.faker.book().publisher()).distinct().map(
					name -> Publisher.New(
						name,
						this.createAddress(countryData.randomCity(this.random), countryData.faker))).collect(toList());
		}
		
		private List<Author> createAuthors(
			final CountryData countryData)
		{
			return this.randomRange(this.dataAmount.maxAuthorsPerCountry()).mapToObj(
				i -> countryData.faker.book().author()).distinct().map(
					name -> Author.New(
						name,
						this.createAddress(countryData.randomCity(this.random), countryData.faker))).collect(toList());
		}
			...
			final int nr)
		{
			...
			final Map<Book, Integer> inventory =
				this.randomRange(this.dataAmount.maxBooksPerShop()).mapToObj(i -> this.randomBook()).distinct().collect(
					toMap(
						book -> book,
			...
			final Set<Customer> customers)
		{
			...
			final List<Purchase> purchases = countries.parallelStream().flatMap(
				country -> country.shops.stream().flatMap(
					shop -> this.createPurchases(country, year, shop))).collect(toList());
			...
			final Shop shop)
		{
			...
			return shop.employees().flatMap(
				employee -> this.randomRange(this.dataAmount.maxPurchasesPerEmployeePerYear()).mapToObj(pi ->
				{
					...
					final List<Purchase.Item> items = this.randomRange(this.dataAmount.maxItemsPerPurchase()).mapToObj(
						ii -> Purchase.Item.New(
							books.get(random.nextInt(books.size())),
							random.nextInt(3) + 1)).collect(toList());
		...
		private List<Employee> createEmployees(
			...
			final City city)
		{
			return this.randomRange(this.dataAmount.maxEmployeesPerShop()).mapToObj(
				i -> Employee.New(
					countryData.faker.name().fullName(),
					this.createAddress(city, countryData.faker))).collect(toList());
		}
