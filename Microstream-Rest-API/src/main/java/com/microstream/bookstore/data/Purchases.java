...
import static com.microstream.bookstore.util.CollectionUtils.ensureParallelStream;
import static com.microstream.bookstore.util.CollectionUtils.maxKey;
import static com.microstream.bookstore.util.CollectionUtils.summingMonetaryAmount;
import static com.microstream.bookstore.util.LazyUtils.clearIfStored;
...
import com.microstream.bookstore.data.Purchase.Item;
import com.microstream.bookstore.util.concurrent.ReadWriteLocked;
import com.microstream.bookstore.util.concurrent.ReadWriteLockedStriped;
...
public interface Purchases
{
	 ...
	 *            the new purchase
	 * @param persister
	 *            the persister to store it with
	 * @see #add(Purchase)
	 */
	 ...
	 *            the return type
	 * @param shopSelector
	 *            predicate for shops to filter by
	 * @param year
	 *            year to filter by
	 * @param streamFunction
	 *            computing function
	...
	public <T> T computeByShopsAndYear(
		...
		int year,
		Function<Stream<Purchase>, T> streamFunction);
	...
	public static class Default extends ReadWriteLockedStriped.Scope implements Purchases
	{
		...
		static class YearlyPurchases
		{
			...
			{
				return this.shopToPurchases.entrySet().parallelStream().filter(
					e -> shopSelector.test(e.getKey())).flatMap(e -> ensureParallelStream(Lazy.get(e.getValue())));
			}
			
			/**
			 * @param employee
			 *            the employee to filter by
		...
		public void clear(
			final int year)
		{
			this.write(year, () -> clearIfStored(this.yearlyPurchases.get(year)).ifPresent(YearlyPurchases::clear));
		}
		...
		public Range<Integer> years()
		{
			return this.read(0, () ->
			{
				final IntSummaryStatistics summary =
					this.yearlyPurchases.keySet().stream().mapToInt(Integer::intValue).summaryStatistics();
			...
			return this.read(year, () ->
			{
				...
				return streamFunction.apply(
					yearlyPurchases == null
						? Stream.empty()
						: yearlyPurchases.shopToPurchases.values().parallelStream().map(l -> l.get()).flatMap(
							List::stream));
			});
		}
		...
		{
			return purchases.flatMap(Purchase::items).collect(
				groupingBy(
					Item::book,
					summingInt(Item::amount))).entrySet().stream().map(
						e -> BookSales.New(e.getKey(), e.getValue())).sorted().collect(toList());
		}
			...
			final int year)
		{
			return this.computeByShopAndYear(
				shop,
				year,
				purchases -> purchases.map(Purchase::total).collect(
					summarizingMonetary(BookStoreDemo.currencyUnit())).getSum());
		}
