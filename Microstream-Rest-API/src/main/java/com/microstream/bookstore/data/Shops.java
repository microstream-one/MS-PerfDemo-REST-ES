...
public interface Shops
{
	...
	public static class Default extends ReadWriteLocked.Scope implements Shops
	{
			...
			final Function<Stream<InventoryItem>, T> function)
		{
			return this.read(
				() -> function.apply(
					this.shops.parallelStream().flatMap(
						shop -> shop.inventory().compute(
							entries -> entries.map(
								entry -> InventoryItem.New(shop, entry.getKey(), entry.getValue()))))));
		}
		
		@Override
		public Shop ofName(final String name)
		{
			return this.read(() -> this.shops.stream().filter(shop -> shop.name().equals(name)).findAny().orElse(null));
		}
