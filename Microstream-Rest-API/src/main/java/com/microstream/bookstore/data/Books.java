...
import com.microstream.bookstore.data.Index.DocumentPopulator;
import com.microstream.bookstore.data.Index.EntityMatcher;
...
public interface Books
{
	 ...
	 *            the new books
	 * @param persister
	 *            the persister to store them with
	 ...
	 * <pre>
	 * long bookCount =
	 * 	compute(books -> books.filter(book -> book.title().startsWith("The")).map(Book::author).distinct().count());
	 * </pre>
	 *
	 * @param <T>
	 *            the return type
	...
	public static class Default extends ReadWriteLocked.Scope implements Books
	{
		 ...
		 *
		 * @param persister
		 *            the MicroStream persister used to store the objects
		 */
		...
		private Index<Book> createIndex()
		{
			...
			final EntityMatcher<Book> entityMatcher = document -> this.isbn13ToBook.get(document.get("isbn13"));
